...
class DatasetCOCO(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, cam_train_path, cam_val_path):
        ...
        self.base_path = os.path.join(datapath, 'MSCOCO2014')
        ...
        # Since all CAM files are in the same folder, use either path
        self.cam_path = cam_train_path
        
        # Ensure the path ends with slash for consistency
        if self.cam_path and not self.cam_path.endswith('/'):
            self.cam_path = self.cam_path + '/'
            
        # Debug info
        print(f"Using CAM path: {self.cam_path}")
        if os.path.exists(self.cam_path):
            cam_files = os.listdir(self.cam_path)
            print(f"Found {len(cam_files)} CAM files in directory")
    ...
    def __getitem__(self, idx):
        ...
        # Extract just the filename from query_name (remove folder prefix like "train2014/")
        # query_name is like "train2014/COCO_train2014_000000040248.jpg"
        query_filename = os.path.basename(query_name)  # This gives "COCO_train2014_000000040248.jpg"
        query_name_base = query_filename.replace('.jpg', '')
        query_cam_filename = f"{query_name_base}--{class_sample}.pt"
        query_cam_path = os.path.join(self.cam_path, query_cam_filename)
        
        # Load query CAM with error handling
        if os.path.exists(query_cam_path):
            query_cam = torch.load(query_cam_path)
        else:
            print(f"Warning: Query CAM file not found: {query_cam_filename}")
            # Create dummy tensor with expected size (50x50 based on your comment)
            query_cam = torch.zeros((50, 50))

        nshot = len(support_names)
        support_cams = []
        for nn in range(nshot):
            # Extract just the filename from support names (remove folder prefix)
            support_filename = os.path.basename(support_names[nn])  # This gives "COCO_train2014_000000504160.jpg"
            support_name_base = support_filename.replace('.jpg', '')
            support_cam_filename = f"{support_name_base}--{class_sample}.pt"
            support_cam_path = os.path.join(self.cam_path, support_cam_filename)
            
            if os.path.exists(support_cam_path):
                support_cam = torch.load(support_cam_path).unsqueeze(0)
            else:
                print(f"Warning: Support CAM file not found: {support_cam_filename}")
                support_cam = torch.zeros((1, 50, 50))
            support_cams.append(support_cam)
        
        # Handle case where no support CAMs were loaded
        if support_cams:
            support_cams = torch.cat(support_cams, dim=0)
        else:
            support_cams = torch.zeros((nshot, 50, 50))
