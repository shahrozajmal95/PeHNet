""" Hypercorrelation Squeeze Network - True Closed Loop Version """
import pdb
...
class HypercorrSqueezeNetwork_imr(nn.Module):
    ...
    def forward(self, query_img, support_img, support_cam, query_cam,
        ...
        if isvgg:
            # For VGG, use the mid-level features directly
            base_mid_feat_idx = 3
            pam_dim = 512
        else:
            # For ResNet, mid-level features are at indices 4-9
            base_mid_feat_idx = 4
            pam_dim = 1024

        bsz = query_img.shape[0]
        H, W = 50, 50  # Fixed spatial size for PAM
        
        # Initial masks for first iteration
        current_query_cam = query_cam.clone()
        current_support_cam = support_cam.clone()
        ...
        for ss in range(stage):
            # ---------- Step 1: Prepare features with current masks ----------
            with torch.no_grad():
                # Extract mid-level features from backbone (these are fixed)
                if isvgg:
                    # VGG: use features[3], features[4], features[5]
                    q_mid_feat_raw = F.interpolate(
                        query_feats[3] + query_feats[4] + query_feats[5],
                        (H, W), mode='bilinear', align_corners=True
                    )
                    s_mid_feat_raw = F.interpolate(
                        support_feats[3] + support_feats[4] + support_feats[5],
                        (H, W), mode='bilinear', align_corners=True
                    )
                else:
                    # ResNet: use features[4] through features[9]
                    q_mid_feat_raw = F.interpolate(
                        query_feats[4] + query_feats[5] + query_feats[6] + \
                        query_feats[7] + query_feats[8] + query_feats[9],
                        (H, W), mode='bilinear', align_corners=True
                    )
                    s_mid_feat_raw = F.interpolate(
                        support_feats[4] + support_feats[5] + support_feats[6] + \
                        support_feats[7] + support_feats[8] + support_feats[9],
                        (H, W), mode='bilinear', align_corners=True
                    )
                
                # Prepare for PAM (flatten spatial dimensions)
                B, C, H_feat, W_feat = q_mid_feat_raw.shape
                q_mid_flat = q_mid_feat_raw.view(B, C, -1).transpose(1, 2)  # [B, 2500, C]
                s_mid_flat = s_mid_feat_raw.view(B, C, -1).transpose(1, 2)  # [B, 2500, C]
                
                # Use CURRENT masks (from previous iteration or initial)
                support_mask_pam = F.interpolate(
                    current_support_cam.unsqueeze(1), (H_feat, W_feat), mode='nearest'
                )
            
            # ---------- Step 2: Apply PAM with current masks (CLOSES THE LOOP) ----------
            ...
            if isvgg:
                q_mid_feat = enhanced_qry.transpose(1, 2).view(B, C, H_feat, W_feat)
                s_mid_feat = enhanced_sup.transpose(1, 2).view(B, C, H_feat, W_feat)
            else:
                # For ResNet, reduce dimensions after PAM
                q_mid_feat = enhanced_qry.transpose(1, 2).view(B, C, H_feat, W_feat)
                s_mid_feat = enhanced_sup.transpose(1, 2).view(B, C, H_feat, W_feat)
                q_mid_feat = self.conv1024_512(q_mid_feat)
                s_mid_feat = self.conv1024_512(s_mid_feat)
            
            # ---------- Step 3: Mask features with current CAMs ----------
            with torch.no_grad():
                query_feats_masked = self.mask_feature(query_feats.copy(), current_query_cam)
                support_feats_masked = self.mask_feature(support_feats.copy(), current_support_cam)
                
                # Compute correlations with current masks
            ...
            imr_x_query = torch.cat([
                current_query_cam.unsqueeze(1), after4d_query, q_mid_feat, state_query
            ], dim=1)
            ...
            imr_x_query_h = self.convh0(
                torch.cat([current_query_cam.unsqueeze(1), after4d_query, q_mid_feat, imr_r_query * state_query], dim=1)
            )
            ...
            imr_x_support = torch.cat([
                current_support_cam.unsqueeze(1), after4d_support, s_mid_feat, state_support
            ], dim=1)
            ...
            imr_x_support_h = self.convh0(
                torch.cat([current_support_cam.unsqueeze(1), after4d_support, s_mid_feat, imr_r_support * state_support], dim=1)
            )
            ...
            logit_mask_support = self.res(
                torch.cat([
                    logit_mask_support, 
                    F.interpolate(current_support_cam.unsqueeze(1), (100, 100), mode='bilinear', align_corners=True)
                ], dim=1)
            )
            logit_mask_query = self.res(
                torch.cat([
                    logit_mask_query, 
                    F.interpolate(current_query_cam.unsqueeze(1), (100, 100), mode='bilinear', align_corners=True)
                ], dim=1)
            )
            
            # ---------- Step 6: Compute loss (if training) ----------
            if query_mask is not None:  # training mode
                if not self.use_original_imgsize:
                    logit_mask_query_temp = F.interpolate(
                        logit_mask_query, support_img.size()[2:], mode='bilinear', align_corners=True
                    )
                    logit_mask_support_temp = F.interpolate(
            ...
            # ---------- Step 7: Update masks for next iteration (CLOSES THE LOOP) ----------
            if ss != stage - 1:
                # Get softmax probabilities for next iteration's masks
                current_query_cam = logit_mask_query.softmax(dim=1)[:, 1].detach()
                current_support_cam = logit_mask_support.softmax(dim=1)[:, 1].detach()
                
                # Reshape for next iteration
                current_query_cam = F.interpolate(
                    current_query_cam.unsqueeze(1), (H, W), mode='bilinear', align_corners=True
                ).squeeze(1)
                current_support_cam = F.interpolate(
                    current_support_cam.unsqueeze(1), (H, W), mode='bilinear', align_corners=True
                ).squeeze(1)
    ...
    def mask_feature(self, features, support_mask):
        """Mask features with the given support mask"""
    ...
    def train_mode(self):
        """Set model to training mode with backbone frozen"""
        self.train()
