...
class Visualizer:
    ...
    def initialize(cls, visualize):
        ...
        # Define green, yellow, and blue colors
        cls.colors = {
            'green': (50, 255, 50), 
            'yellow': (255, 255, 50),
            'blue': (102, 140, 255)
    ...
    def visualize_prediction(
        ...
        # Assign colors: blue for support, green for prediction, yellow for ground truth
        spt_color = cls.colors['blue']        # Support images
        pred_color = cls.colors['green']      # Model predictions
        qry_color = cls.colors['yellow']      # Query ground truth
        ...
        # Apply green mask to predictions
        pred_masked_pil = Image.fromarray(
            cls.apply_mask(qry_img.astype(np.uint8), pred_mask.astype(np.uint8), pred_color))
        
        # Apply yellow mask to ground truth
        qry_masked_pil = Image.fromarray(
            cls.apply_mask(qry_img.astype(np.uint8), qry_mask.astype(np.uint8), qry_color))

        # Combine all images: support images, prediction, ground truth
    ...
    def merge_image_pair(cls, pil_imgs):
        """ Horizontally aligns images and returns PIL object """
